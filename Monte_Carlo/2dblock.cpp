...
void block::rdf(string filename){
    stringstream st;
    const double omega = pow(rMin,2)*(w-1)*(h-1)/n;
    //const double omega = pow(rMin,2)*(800-1)*(800-1)/n;
...
//-------------------------HW 5 starts from here  -------------------------
//=========================================================================
void block::Move(double alpha, int &sidx,double &oriXpos, double &oriYpos){
    //first randomly pick an atom from the 400 atom block:
    int ridx = rand()%int(400); //ridx is in range 0 to 399;
    sidx = ridx;

    //next, generate two random numbers zetax and zetay
    double zetax = -1.0 + 2.0*(double)rand()/ RAND_MAX;
    double zetay = -1.0 + 2.0*(double)rand()/ RAND_MAX;

    oriXpos = atomlist[ridx][0];
    oriYpos = atomlist[ridx][1];

    //now move that atom by: x' = x + alpha*zetax;   y' = y + alpha*zetay;
    atomlist[ridx][0] += alpha*zetax;
    atomlist[ridx][1] += alpha*zetay;
    //cout<<"xmove = "<<alpha*zetax <<"ymove = "<<alpha*zetay<<endl;
}

void block::redoMove(int sidx, double oriXpos, double oriYpos){
    atomlist[sidx][0] = oriXpos;
    atomlist[sidx][1] = oriYpos;
}

void block::MonteCarlo(long int steps,double T,string dir){
    stringstream st;
    long int it = 0;
    // double KE = 400*k_b*T;
    double PE = 0;
    ...
    while(it<steps){
        PE = total_penergy();
        double oriXpos,oriYpos,newPE,probaccept;
        double prob;
        int sidx;
        // move the atom
        ...
        if(counter > steps/500){
            ...
            // adjust the alpha value as try to make probacceptave to be in 
            ...
            cout <<"Ave alpha = \t"<<sumalpha<<"Accept prob \t: "<<probacceptave<<"PE \t"
                <<PEave<<"PEnew \t "<<newPE<<"prob \t"<<prob<<endl;
            counter = 0;
            st <<PEave<<" "<<probacceptave<<" "<<sumalpha<<endl;
        }

        // record the energy data:
        // st << PE <<" "<<probaccept<<endl;
        counter++;
        it++;
    }
    // last update the force and stress:
    get_atomic_stress();

    string filename = dir +"/Energy.txt";
    ofstream myfile(filename);
    string data = st.str();
    myfile<< data;
    myfile.close();
...
void block::CollectEqdata(long int steps,double T,string dir){
    stringstream st;
    long int it = 0;
    double KE = 400*k_b*T;
    double PE = 0;
    ...
    string filename = dir +"/Equilibrium.txt";
    ofstream myfile(filename);
    string data = st.str();
    myfile<< data;
    myfile.close();
